...
var PersonSchema = new Schema({
    name: {type: String, required: true, default: "Pat"},
    age: {type: Number, default: 29},
    favoriteFoods: {type: [String], default:['pizza','cheese']},
})
...
var queryChain = function(done) {
  ...
  var foods = Person.find({favoriteFoods: foodToSearch});
  var sorted = foods.sort({name: 'asc'});
  var limit = foods.limit(2)
  var select = foods.select({age: 0})
  // Person.find({favoriteFoods: foodToSearch}).sort({name: 'asc'}).limit(2).select({age: 0}).exec(function(err, data) {
  //   if (err) return done(err)
  //   return done(null, data)
  // })
  foods.exec(function(err, data) {
    if (err) return done(err)
    return done(null, data)
  })
  //done(null/*, data*/);
